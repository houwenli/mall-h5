<template>
    <div>
        ...
        <van-share-sheet v-model="showSharePopup" class="custome-share" title="分享图片至" :close-on-click-overlay="true" :lock-scroll="false" :options="shareSheetOptions" @select="onSelectShare" @cancel="onCancelShare" @click-overlay="clonse"></van-share-sheet>
    ...
    methods: {
        ...
        onSelectShare(option) {
            ...
            if (this.postSpuName.length > 30) {
                this.postSpuName = this.postSpuName.length.substr(0, 30) + '…'
            }
            let iconImgUrl = `${this.spu.images[0]}?imageView2/1/w/60`
            const url = `${baseUrl}/motorCar/detail?skuId=${this.spu.id}&storeId=${this.$route.query.storeId || ''}&sourceType=${this.$route.query.sourceType || 1}`
            const comParasm = {
                url,
                title: skuName,
