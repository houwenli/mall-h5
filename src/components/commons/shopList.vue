<template>
  <div class="guess-warp">
    ...
    <div class="guess-title" v-if="title">
      <img src="@/assets/img/goodsDetail/hotSell.png" />
    </div>
    <!-- 是否有筛选条件 -->
    ...
    <!-- 是否有列表信息 -->
    <div v-if="!isOld">
      <waterfall class="waterfall" ref="waterfall" :data="shopList" :storeId="storeId" :isEmpty="isEmpty"  :noMore="noMore" :detailShow="detailShow" :botTip="botTip" :isDistribution="isDistribution"/>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    /**
     * 查询热销榜单和热销广告
     * 仅在首页tab里面的  一级分类的 【首页】展示
     */
    async getHotAdAnd() {
      if (this.isIndex === false) return

      try {
        const channelType = this.$store.state.userSource == 1 ? 4 : this.$store.state.userSource == 2 ? 5 : 2

        // 上面 isIndex为true 表示当前的路由页面为首页 + classifyActiveTab = 0 ， 则表示为首页-的一级分类的首页
        // classifyActiveTab 不为0； 则表示为 其他的一级分类
        if (this.classifyActiveTab !== 0) return

        const list = await queryAdvertSet({channelType})
    ...
    getData: debounce(function () {
      ...
      if (this.storeId) {
        ...
        let routePath = {
          '/':1, //首页底部的热销商品
          '/spudetail':2,  //商品详情页
          '/collectionSpudetail':2,  //集采商品详情页
          '/cart':3, //购物车
          '/home':4, //个人中心
          '/paysuccess':5, //支付后回调页
          '/orderlist':6, //订单列表
          '/orderSearchList':6, //订单搜索结果列表
          '/orderdetail':7, //:订单详情页
          '/ranking': this.pageType, //:订单详情页
          '/tracesOrder': 6, // 物流页面 和订单列表页一样
        }
        let params = {
        ...
        try {
          getRecommendList(params).then(async(response) => {
