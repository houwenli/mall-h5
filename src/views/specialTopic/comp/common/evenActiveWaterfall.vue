    ...
    <div v-if="!isEmpty">
      <div class="guess-list">
        ...
        <div class="column-container left" ref="col-left">
          <goodList v-for="(item, index) in leftData" ref="goodList" :item="item" :key="index" :detailShow="detailShow"></goodList>
        </div>
        <div class="column-container right" ref="col-right">
          <goodList v-for="(item, index) in rightData" ref="goodList" :item="item" :key="index" :detailShow="detailShow"></goodList>
        </div>
      </div>
      <div class="guess-load-more-tips" v-if="noMore" ref="backTop" :style="botTip=='见底啦~不如去'?'font-size: 13px;':''">
        {{ botTip }}
        <div class="go-home" @click="$router.push({ path: '/' })" v-if="botTip=='见底啦~不如去'"> 
            首页逛逛
        </div>
      </div>
