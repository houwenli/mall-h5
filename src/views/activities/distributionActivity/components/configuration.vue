...
<!--  v-if="isShowHomePageConfig" -->
  <div class="home_page">
    <div class="header_nav">
      <header-nav ref="headerNav" :showLogo="showLogo" :bgColorIndex="bgColorIndex" :isInTop="isInTop" :headerData="headerData"></header-nav>
    </div>
   <div class="content wrap-content-main" ref="contentTop">
      <div
        ref="backTop"
        :style="{
          height: '100%',
      ...
      >
        ...
        <div class="list-group-item" v-for="element in contentList" :key="element.id">
          <template v-if="element.text === '轮播图'">
            <swipe :listData="element"></swipe>
          </template>
          <template v-if="element.text === '单列图'">
            <group-image :listData="element.list" :row="1"></group-image>
          </template>
          <template v-if="element.text === '双列图'">
            <group-image :listData="element.list" :row="2"></group-image>
          </template>
          <template v-if="element.text === '三列图'">
            <group-image :listData="element.list" :row="3"></group-image>
          </template>
          <template v-if="element.text === '四列图'">
            <group-image :listData="element.list" :row="4"></group-image>
          </template>
          <template v-if="element.text === '滑动区块'">
            <slide-module :listData="element.list"></slide-module>
          </template>
          <template v-if="element.text === '图片魔方'">
            <cube-image :listData="element.list"></cube-image>
          </template>
          ...
          <template v-if="element.text === '公告'">
            <notice-bar :listData="element.list"></notice-bar>
          </template>
          <template v-if="element.text === '辅助空白'">
            <blank :listData="element.list"></blank>
          </template>
        </div>
        <div class="mobile-template">
          <p class="mobile-template_p">精选好物</p>
          <shop-list @shareComm="(item)=>{$refs.distributionShare.shareComm(1, item)}"></shop-list>
        </div>
      </div>
    </div>
    ...
    <!-- <div class="returntopBox" v-if="isshowtop">
      <img src="@/assets/img/top@2x.png" class="returntop" @click="backtopActive" alt="" />
    </div> -->


    <!-- 分享 -->
    <img class="img_top" src="@/assets/img/distribution/weChat.png" @click.stop="$refs.distributionShare.shareComm(2,null)" v-if="!isSharedPage" />
    <!-- 回到顶部  -->
    <img class="img_to_top" src="@/assets/img/distribution/img_to_top.png" v-if="isshowtop" @click.stop="backtopActive" />
    <!-- 分享弹框 -->
...
<script>
  ...
  import shopList from '@/components/commons/distributionShopList'
  ...
  // ---------------------
  import headerNav from '@/components/index/template/header/distributionSearch.vue'
  import swipe from '../template/swipe'
  import HeaderSwiper from '../template/header/header-swiper.vue'
  import cubeImage from '../template/cubeImage'
      ...
      queryMobileIndex() {
        queryDistributionHomeTemplate().then((res) => {
          if (res.code === 200) {
            ...
            this.pageNavData = data.pageNavData
